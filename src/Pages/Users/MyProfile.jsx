...
function MyProfile() {
  ...
  const fetchAddresses = async () => {
    try {
      const res = await axios.get(
        `${BASE_API_URL}/address/my-address`,
        {
          headers: { Authorization: `Bearer ${accessToken}` },
        }
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const apiUrl = editMode
        ? `${BASE_API_URL}/address/edit-address/${selectedAddressId}`
        : `${BASE_API_URL}/address/new-address`;
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await axios.delete(
        `${BASE_API_URL}/address/delete-address/${id}`,
        { headers: { Authorization: `Bearer ${accessToken}` } }
      );
  ...
  const handlePasswordSubmit = async (e) => {
    ...
    try {
      const res = await axios.patch(
        `${BASE_API_URL}/users/${user.id}/reset-password`,
        {
  ...
  const handleEditShow = () => {
    setEditData({
      f_name: myUser.f_name,
      l_name: myUser.l_name,
      email: myUser.email,
      contact: myUser.contact,
    });
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const res = await axios.patch(
        `${BASE_API_URL}/users/my-user`,
        { ...editData, id: user.id },
        { headers: { Authorization: `Bearer ${accessToken}` } }
      );

      setMessage(res.data.message || "Profile updated successfully!");
      setMyUser((prev) => ({ ...prev, ...editData }));
      handleEditClose();
    } catch (err) {
