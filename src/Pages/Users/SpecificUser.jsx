...
import { Col, Row, Card, Badge, Spinner, Alert, Button, Modal, Form, Container } from 'react-bootstrap';
import Sidebar from '../../Components/SideBar/SideBar';
import { AuthContext } from '../../context/AuthContext';
import axios from 'axios';
import moment from 'moment';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faTrash, faPlus, faChevronRight } from '@fortawesome/free-solid-svg-icons';
...
function SpecificUser() {
    ...
    const fetchUser = async () => {
        ...
        try {
            ...
            if (err.response?.status === 404) setError('The requested user could not be found.');
            else if (err.response?.status === 403) setError('Access Denied: You are not authorized to view this user.');
        ...
        return (
            <>
                ...
                <Row className="vh-100">
                    ...
                    <Col xs={10} md={10} className="bg-light d-flex justify-content-center align-items-center">
                    ...
                    <Col xs={10} md={10} className="bg-light pt-5 mt-3">
                        <Container>
                            <Alert variant="danger" className="shadow-sm">
                                <Alert.Heading className="h4">Error Retrieving Data</Alert.Heading>
                                <p>{error}</p>
                                <hr />
        ...
        <>
                    ...
                    <Container fluid className='form-box'>
                        ...
                        <Row className="mb-4">
                            <Col md={6}>
                                <Card className="shadow-sm border-0 h-100">
                                    ...
                                    <Card.Body className="p-4">
                                        ...
                                        <p className="h6 text-dark mb-0">{moment(user.created_at).format('MMMM Do YYYY')}</p>
                                    </Card.Body>
                                </Card>
                            </Col>
                            ...
                            <Col md={6}>
                                <Card className="shadow-sm border-0 h-100">
                                    <Card.Header className="bg-white border-bottom py-3">
                                        <h5 className="mb-0 text-dark fw-bold">Role & Organization</h5>
                                    </Card.Header>
                                    <Card.Body className="p-4">
                                        <p className="text-muted small mb-1">User Role</p>
                                        <Badge bg="info" className="px-3 py-2">{user.role}</Badge>
                                        <p className="text-muted small mb-1 mt-3">Organization</p>
                                        <p className="h6 text-dark mb-0">{user.org_name || 'N/A'}</p>
                                    </Card.Body>
                                </Card>
                            </Col>
                        </Row>
                                        ...
                                        <div className="d-flex flex-wrap gap-2">
                                            {userGroups.length > 0 ? (
                                                userGroups.map((group, index) => (
                                                    <Badge key={index} bg="light" text="dark" className="px-3 py-2 d-flex align-items-center gap-2">
                                                        {group}
                                                        <button
                                                ...
                                                <p className="text-muted small">No groups assigned yet.</p>
                                            )}
                                        </div>
                        ...
                        <Row className="mb-4">
                            <Col md={12}>
                                <div className="d-flex gap-3 justify-content-end">
                                    <Button variant="outline-secondary" onClick={() => navigate(`/${org_id}/users`)}>
                                        Back to Users
                                    </Button>
            ...
            <Modal show={showDeleteModal} onHide={() => setShowDeleteModal(false)} centered>
                ...
                <Modal.Body className="py-4">
                    <p className="mb-2">Are you sure you want to delete <strong>{user?.f_name} {user?.l_name}</strong>?</p>
